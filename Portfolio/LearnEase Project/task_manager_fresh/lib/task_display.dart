            ...
            trailing: Row(
              ...
              children: [
                IconButton(
                  ...
                  onPressed: () async {
                    final links = await sortingService.getYouTubeRecommendations(t['task'] as String);
                    if (!context.mounted) return;
                    showModalBottomSheet(
                      context: context,
                      builder: (_) => ListView(
                        padding: const EdgeInsets.all(12),
                        children: links.map((url) => SelectableText(url, style: const TextStyle(fontSize: 13))).toList(),
                      ),
                    );
                  },
                ),
                IconButton(
                  ...
                  tooltip: isSchedule ? 'Remove scheduled session' : 'Remove task',
                  onPressed: () => onRemoveTask(t),
                ),
              ],
            ),
            onTap: () async {
              final edited = await showDialog<Map<String, dynamic>>(
                ...
                builder: (ctx) => Dialog(
                  child: TaskForm(
                    onAddTask: (task, date, diff) async {
                      Navigator.pop(ctx, {'task': task, 'date': date, 'difficulty': diff});
                    },
                    initialTask: t,
                  ),
                ),

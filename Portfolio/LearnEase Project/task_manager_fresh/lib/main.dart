...
class _MyHomePageState extends State<MyHomePage> with TickerProviderStateMixin {
            ...
            if (m.containsKey('sessions'))
              'sessions': (m['sessions'] as List).map((s) {
                return {
                  ...
                  'title': s['title'] as String,    // ← restore the title
                };
              }).toList(),
      ...
      if (t.containsKey('sessions')) {
        m['sessions'] = (t['sessions'] as List).map((s) {
          return {
            ...
            'title': s['title'] as String,      // ← persist the AI-generated title
          };
        }).toList();
      }
      return m;
  ...
  Future<void> _addTask(String title, DateTime due, String diff) async {
    // 1️⃣ Copy existing tasks into a new list
    final allTasks = List<Map<String, dynamic>>.from(_tasks);

    // 2️⃣ Append the new task (no sessions yet)
    allTasks.add({
    ...
    // 3️⃣ **Full reschedule:** regenerate sessions for *every* task
    for (final t in allTasks) {
      final blocks = await scheduler.getSuggestedSchedule(
        t['task'] as String,
        t['date'] as DateTime,
        t['difficulty'] as String,
        allTasks,       // pass the full list for conflict awareness
        _classRecords,  // your class schedule list
      );
    ...
    setState(() => _tasks = allTasks);
    await _saveTasks();  // your existing write-to-storage helper
  }
  ...
  Widget _buildGroup(List<Map<String, dynamic>> source,
      {bool isSchedule = false}) {
    if (source.isEmpty) return const Center(child: Text('Nothing here'));

    // 1) In Schedule mode, turn each task's sessions into separate items:
    final List<Map<String, dynamic>> items = isSchedule
        ? [
      for (final task in source)
        if (task['sessions'] != null)
          for (final s in task['sessions'] as List)
            {
              'id':
              '${task['id']}_${(s['start'] as DateTime).toIso8601String()}',
              'task': s['title'] ?? task['task'],
              'difficulty': task['difficulty'],
              'start': s['start'] as DateTime,
              'end': s['end'] as DateTime,
              'parent': task,
            }
    ]
        : source;

    // 2) Group items by the calendar date of their start (or task.date)
    final grouped = <String, List<Map<String, dynamic>>>{};
    for (final m in items) {
      final dt = isSchedule ? m['start'] as DateTime : m['date'] as DateTime;
    ...
    // 3) Render each date section
    return ListView(
      children: [
        for (final k in sortedDates) ...[
          // Date header
          Container(
            ...
            child: Text(
              DateFormat('MMM dd').format(DateTime.parse(k)),
          ...
          // TaskDisplay shows only this date's items
          TaskDisplay(
            tasks: grouped[k]!,
            ...
            onRemoveTask: (item) async {
              if (isSchedule) {
                // remove only that one session
                final parent = item['parent'] as Map<String, dynamic>;
                setState(() {
                  parent['sessions']
                      .removeWhere((s) => (s['start'] as DateTime) == item['start']);
                });
              } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavigationBar(
        currentIndex: _nav,
        onTap: (i) async {
          // Cancel any running ad‐timer if leaving Chatbot
          if (i != 4 && _chatAdTimer != null) {
            _chatAdTimer!.cancel();
            _chatAdTimer = null;
          }

          // When switching into Chatbot tab (index 4):
          if (i == 4 && _nav != 4) {
            final now = DateTime.now();
            // Show first ad immediately if 3 min have passed
            if (_lastChatAd == null ||
                now.difference(_lastChatAd!) >= Duration(minutes: 3)) {
              await showDialog(
                context: context,
                barrierDismissible: false,
                builder: (_) => AdDialog(duration: Duration(seconds: 30)),
              );
              _lastChatAd = now;
            }
            // Start a repeating 3-minute timer for subsequent ads
            _chatAdTimer = Timer.periodic(Duration(minutes: 3), (_) async {
              if (_nav == 4) {
                await showDialog(
                  context: context,
                  barrierDismissible: false,
          ...
          setState(() {
            _nav = i;
            if (i != 0) _showDailySchedule = false;
          });
        },
        items: const [
